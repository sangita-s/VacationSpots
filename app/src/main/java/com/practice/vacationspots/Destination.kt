...
@Entity(tableName = "destinations", indices = [Index(value = ["placeName"], unique = true)])
data class Destination(@PrimaryKey val id: String, val imageId: Int, val placeName: String)
...
object VacationSpots {
    ...
    private val cityNames = arrayListOf(
        ...
        "Kakadu National Park", "Melbourne", "Great Barrier Reef", "Sydney"
    )
    ...
    var list: ArrayList<Destination>? = null
        get() {
            ...
            for (i in images.indices) {
                ...
                val destination = Destination(id, imageId, cityName)
